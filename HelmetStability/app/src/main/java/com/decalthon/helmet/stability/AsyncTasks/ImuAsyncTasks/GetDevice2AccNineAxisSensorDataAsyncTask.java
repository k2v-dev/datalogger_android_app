...
public class GetDevice2AccNineAxisSensorDataAsyncTask extends AsyncTask<Long,
        Void, List<SensorDataEntry>> {
    @Override
    protected List<SensorDataEntry> doInBackground(Long... longs) {
        return SessionCdlDb.getInstance().getSessionDataDAO().getDevice2AccelerometerData(longs);
    }
}
