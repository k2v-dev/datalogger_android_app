...
public class GetDevice2MagnetoNineAxisSensorDataAsyncTask extends AsyncTask<Long,
        Void, List<SensorDataEntry>> {
    @Override
    protected List<SensorDataEntry> doInBackground(Long... longs) {
        return SessionCdlDb.getInstance().getSessionDataDAO().getDevice2MagnetometerData(longs);
    }
}
